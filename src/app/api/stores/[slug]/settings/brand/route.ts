...
import { updateBrandSettingsBodySchema } from "@/lib/schemas/store";
  ...
  try {
    ...
    if (!isOwner) {
      ...
      if (member?.role !== "admin") {
        return forbidden("Only owners and admins can update brand settings.");
      }
    }

    storeId = ctx.storeId;
    const parsedBody = await parseJson(request, updateBrandSettingsBodySchema);
    ...
    const [updated] = await db
      .update(stores)
      .set({
        ...(body.storeName !== undefined ? { name: body.storeName } : {}),
    ...
    await logRouteError("Error updating brand settings", error, params, { storeId });
    return serverError("Failed to update brand settings");
  }
}
