...
export function ConversionFunnelChart({ data }: ConversionFunnelChartProps) {
  const baseFunnelColor = "bg-muted/60";

  const steps = [
    {
      label: "Product Views",
      value: data.views,
      color: baseFunnelColor,
      conversion: "100%",
    },
    {
      label: "Add to Cart",
    ...
    },
    {
      ...
      color: baseFunnelColor,
      conversion:
        data.addToCarts > 0 ? `${((data.checkouts / data.addToCarts) * 100).toFixed(1)}%` : "0%",
    },
    {
      ...
      color: baseFunnelColor,
      conversion:
        data.checkouts > 0 ? `${((data.purchases / data.checkouts) * 100).toFixed(1)}%` : "0%",
    },
  ];
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Track how customers move through your purchase journey</CardDescription>
      </CardHeader>
      <CardContent>
              ...
              <div key={step.label} className="space-y-2">
                ...
                <div className="relative">
                  <div className="w-full bg-muted rounded-full h-2">
                    <div
                      className={cn("h-2 rounded-full transition-all duration-300", step.color)}
